...
var _ = Describe("KonfluxSegmentBridge Controller", func() {
	Context("When reconciling a resource", func() {
		...
		It("should create Secret with only TEKTON_RESULTS_API_ADDR when no write key is configured", func() {
			controllerReconciler := &KonfluxSegmentBridgeReconciler{
				Client:               k8sClient,
				Scheme:               k8sClient.Scheme(),
				ObjectStore:          objectStore,
			...
			Expect(string(secret.Data["SEGMENT_BATCH_API"])).To(BeEmpty(), "batch API should be empty")
		})
