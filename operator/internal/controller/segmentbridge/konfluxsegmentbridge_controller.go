...
// endpoint). When a Segment write key is available, also includes SEGMENT_WRITE_KEY and
// SEGMENT_BATCH_API.
//
...
//  3. Empty -- Secret is still created with TEKTON_RESULTS_API_ADDR so the CronJob
//     can reach the Results API; the segment-bridge scripts handle the missing key
//     gracefully by skipping the upload step.
func (r *KonfluxSegmentBridgeReconciler) reconcileSegmentBridgeSecret(ctx context.Context, tc *tracking.Client, spec *konfluxv1alpha1.KonfluxSegmentBridgeSpec) error {
	...
	if segmentKey != "" {
		...
		data["SEGMENT_BATCH_API"] = batchURL
	}
